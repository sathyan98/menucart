...
const Menuinfo = (props) => {
  ...
  useEffect(() => {
    fetch(`https://api.spoonacular.com/food/menuItems/${id}?apiKey=${Apikey}`)
      .then((res) => res.json())
            ...
            {data && (
              <Card style={{ height: "600px", flexBasis: "50%" }}>
                ...
                <CardBody style={{ flexShrink: "2" }}>
                  ...
                  <Row style={{ justifyContent: "center", paddingTop: "5px" }}>
                    <CardText>
                      Total Calories Included for {data?.images.length}{" "}
                      {data?.title} are {data?.nutrition.calories}{" "}
                    </CardText>
                  </Row>
                  <Row style={{ justifyContent: "center", paddingTop: "5px" }}>
                    <CardText>
                      Total Calories Included for {data?.images.length}{" "}
                      {data?.title} are {data?.nutrition.protein}{" "}
                    </CardText>
                  </Row>
                  <Row style={{ justifyContent: "center", paddingTop: "5px" }}>
                    <CardText>
                      Total Calories Included for {data?.images.length}{" "}
                      {data?.title} are {data?.nutrition.fat}{" "}
                    </CardText>
                  </Row>
                  <Row style={{ justifyContent: "center", paddingTop: "5px" }}>
                    <CardText>
                      Total Calories Included for {data?.images.length}{" "}
                      {data?.title} are {data?.nutrition.carbs}{" "}
                    </CardText>
                  </Row>
                </CardBody>
                <Link to={"/cart"}>
